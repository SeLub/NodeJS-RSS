...
const parseConfig = (param) => {
	...
	try {
		...
		param.split('-').forEach(el => {
        	...
        	if (el.length > 2) { throw new ValidationError(`Error in cipher code string : \n  chipher code string must be devided by '-',\n  Exemple: -c 'A-C1-R0'\nYou enter: ${param}\nWrong case: ${el}`); }

			else if ( [0,1].includes(normaliseNumber) && ['C','R','A'].includes(el.charAt(0).toString().toUpperCase()) ) {
